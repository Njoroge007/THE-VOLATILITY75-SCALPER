...
import {
  ...
  SESSION_TTL_HOURS, getDiagLog, type DiagEntry,
...
const TIER_MIN: Record<SignalTier, number> = {
  REJECT: 0, WATCH: 20, CANDIDATE: 30, TRADE: 40, PREMIUM: 55,
};
...
function ProbabilityGauge({ prob, tier }: { prob: number; tier: SignalTier }) {
  ...
  const ranges = [
    { lo: 0,  hi: 20,  color: "#3f3f46",  label: "REJECT" },
    { lo: 20, hi: 30,  color: "#64748b",  label: "WATCH" },
    { lo: 30, hi: 40,  color: "#f59e0b",  label: "CANDIDATE" },
    { lo: 40, hi: 55,  color: "#10b981",  label: "TRADE" },
    { lo: 55, hi: 100, color: "#a78bfa",  label: "PREMIUM" },
  ];
  return (
    <div className="space-y-3">
      ...
      <div className="flex justify-between text-[9px] text-zinc-600 font-mono">
        <span>0</span>
        <span className="text-zinc-500">20 WATCH</span>
        <span className="text-amber-600">30</span>
        <span className="text-emerald-600">40 TRADE</span>
        <span className="text-violet-500">55 PREMIUM</span>
        <span>100</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
        ...
        {(authStatus === "not-connected" || authStatus === "error") && (
          <>
            <div className="flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${authStatus === "error" ? "bg-rose-500" : "bg-zinc-500"}`} />
              <span className={`text-xs font-semibold ${authStatus === "error" ? "text-rose-400" : "text-zinc-400"}`}>
                {authStatus === "error" ? "Auth Error" : "Not Connected"}
              </span>
            </div>
            {authError && (
            ...
            <button
              onClick={onLogin}
              className="flex items-center justify-center gap-2 w-full py-3 px-4 rounded-lg bg-[#ff444f] hover:bg-[#e03040] active:scale-95 transition-all text-white text-sm font-bold shadow-lg shadow-rose-900/40"
            >
              {/* Deriv "D" wordmark icon */}
              <svg width="16" height="16" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true">
                <path d="M4 4h12c8.837 0 16 7.163 16 16s-7.163 16-16 16H4V4zm12 24c6.627 0 12-5.373 12-12S22.627 4 16 4H12v24h4z"/>
              </svg>
              Login with Deriv
            </button>
            <p className="text-[10px] text-zinc-600 text-center">
              You'll be redirected to Deriv's secure login, then back here automatically.
            </p>
          </>
        )}
        {authStatus === "connecting" && (
          <div className="space-y-2">
            ...
            <p className="text-[10px] text-zinc-500">Authorizing with Deriv via WebSocket…</p>
          </div>
        )}
        {/* ── Diagnostics ───────────────────────────────────────────── */}
        {diagLog.length > 0 && (
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 space-y-3">
        ...
        <p className="text-[10px] text-zinc-500">
          {executionMode === "AUTO"
            ? "Trades fire automatically at ≥40% when L3 structure + L4 flow confirm direction (TRADE tier)."
            : "Signal highlights Execute button for 10s when composite ≥40% and layers confirm trend."}
        </p>
      </div>
...
export function V75Analyzer() {
  ...
  useEffect(() => {
    (async () => {
      console.log("[Auth Effect] useEffect fired — checking for OAuth callback");
      console.log("[Auth Effect] window.location.href:", window.location.href);
      console.log("[Auth Effect] window.location.search:", window.location.search);

      // 1. Check for fresh Deriv redirect (acct1/token1 in URL)
      const result = handleOAuthCallback(API);
      console.log("[Auth Effect] handleOAuthCallback result:", result.status);

      if (result.status === "error") {
        console.log("[Auth Effect] OAuth callback ERROR:", result.message);
      ...
      if (result.status === "connected") {
        console.log("[Auth Effect] OAuth token captured — calling authorizeAndGetAccounts");
        setAuthStatus("connecting");
        setDiagLog(getDiagLog());
        try {
          const data      = await authorizeAndGetAccounts(result.token, API);
          const primaryId = data[0]?.id ?? "";
          console.log("[Auth Effect] authorizeAndGetAccounts SUCCESS — primaryId:", primaryId);
          setAccounts(data);
          setSelectedAccountId(primaryId);
          selectedAccountRef.current = primaryId;
          saveSession(result.token, data, primaryId);
          setSessionExpiresAt(Date.now() + SESSION_TTL_HOURS * 60 * 60 * 1000);
          setAuthStatus("connected");
          setDiagLog(getDiagLog());
        } catch (e: any) {
          console.log("[Auth Effect] authorizeAndGetAccounts FAILED:", e?.message);
          clearSession();
          clearAccessToken();
          setAuthStatus("not-connected");
          setAuthError(e?.message ?? "Failed to load accounts after OAuth redirect.");
          setDiagLog(getDiagLog());
        }
        return;
      }
      ...
      console.log("[Auth Effect] No OAuth callback — checking localStorage for saved session");
      const stored = loadSession();
      if (!stored) {
        console.log("[Auth Effect] No saved session found — waiting for login");
        return;
      }

      console.log("[Auth Effect] Saved session found — restoring token and re-verifying");
      ...
      try {
        ...
        console.log("[Auth Effect] Session restore SUCCESS — primaryId:", primaryId);
        ...
        console.log("[Auth Effect] Session restore FAILED:", e?.message);
        clearSession();
        clearAccessToken();
        setAuthStatus("not-connected");
      ...
      <div className={`p-5 flex gap-5 ${activeTab === "BACKTEST" ? "hidden" : ""}`}>
            ...
            <div className="space-y-1.5">
              ...
              <div className={`text-center text-xs font-semibold tracking-widest py-2.5 rounded border mt-2 ${isTradeTier ? "text-emerald-300 bg-emerald-500/10 border-emerald-500/20" : "text-zinc-500 bg-zinc-800/40 border-zinc-700/30"}`}
                ...
                {isTradeTier
                  ...
                  : ready ? `SCANNING — ${probabilityScore.toFixed(0)}% (NEED ≥40%)` : "WARMING UP"}
              </div>
            </div>
          </div>
        ...
        <div className="w-72 shrink-0">
          <AuthPanel
            ...
            onLogin={handleLogin}
            onLogout={() => { clearSession(); clearAccessToken(); setAuthStatus("not-connected"); setAccounts([]); setSelectedAccountId(""); setAuthError(""); setSessionExpiresAt(null); setDiagLog([]); }}
            execError={execError}
