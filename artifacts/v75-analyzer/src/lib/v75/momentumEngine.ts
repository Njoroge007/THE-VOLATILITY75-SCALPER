...
// Thresholds calibrated for V75 — real composite scores peak ~55-65%
// due to V75's aggressive volatility dampening individual layer scores.
const TIER_MIN: Record<SignalTier, number> = {
  PREMIUM:   55,   // Strong confluence across all 5 layers
  TRADE:     40,   // Confirmed entry: layers agree on direction
  CANDIDATE: 30,   // Moderate confluence — approach with caution
  WATCH:     20,   // Developing signal
  REJECT:    0,
};
...
export default class MomentumEngine {
  ...
  processTick(price: number, ms: number, atr: number): Signal | null {
    ...
    const { probabilityScore, tier, flowDirection, layerScores, zScore, hurstExponent, tickVelocity, dti, hurstRegime } = this.lastMetrics;

    // Fire at TRADE tier (≥40%) — calibrated for V75's real score range.
    // Gate: composite ≥40% AND L4 flow confirms a direction AND L3 structure
    // shows conviction (score >50 means structural bias is present, not noise).
    if (tier === "REJECT" || tier === "WATCH" || tier === "CANDIDATE") return null;
    if (flowDirection === "NEUTRAL") return null;
    if (layerScores.structure < 50) return null;
